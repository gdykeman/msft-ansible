...
# Front-end cert
Get-ChildItem -Path $(New-SelfSignedCertificate -dnsname company.local).pspath | Export-PfxCertificate -FilePath "C:\scripts\frontend.pfx" -Password $(ConvertTo-SecureString -String "password1" -Force -AsPlainText)

# Back-end cert
$cert = Get-ChildItem -Path $(New-SelfSignedCertificate -dnsname company.local).pspath
Export-PfxCertificate -Cert $cert -FilePath "C:\scripts\backend.pfx" -Password $(ConvertTo-SecureString -String "password1" -Force -AsPlainText)
